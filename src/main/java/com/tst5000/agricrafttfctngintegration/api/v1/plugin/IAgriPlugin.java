...
import com.tst5000.agricrafttfctngintegration.api.v1.mutation.IAgriCrossStrategy;
...
/**
 * Interface for mod plugins to AgriCraftTFCTNGIntegration.
 * <p>
 * All classes implementing this interface <em>must</em> have a valid no-args constructor, with
 * which AgriCraftTFCTNGIntegration may create the plugin instance.
 * <p>
 * Plugin instantiation occurs during the pre-init phase of FML, at which time very little of
 * AgriCraftTFCTNGIntegration has been initialized. It is strongly recommended that you do not attempt to AgriCraftTFCTNGIntegration
 * in this method, as AgriCraftTFCTNGIntegration will be in an indeterminate state.
 * <p>
 * Plugin initialization occurs during the init phase of FML, at which point, AgriCraftTFCTNGIntegration is only
 * partially initialized. Consequently, proceed with extreme caution in referencing other parts of
 * the mod at such time, given that AgriCraftTFCTNGIntegration will still be in an indeterminate state.
 * <p>
 * Registrations occur during the post-init phase of FML, at which point AgriCraftTFCTNGIntegration is considered
 * functionally initialized. Thereby, this is the first phase during which it is considered
 * relatively safe to access AgriCraftTFCTNGIntegration, so attempt to make connections to AgriCraftTFCTNGIntegration following the
