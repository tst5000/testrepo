package com.tst5000.agricrafttfctngintegration.tiles;
...
import com.tst5000.agricrafttfctngintegration.api.v1.misc.IAgriDisplayable;
...
public class TileEntityCrop extends TileEntityBase implements IAgriCrop, IDebuggable, IAgriDisplayable {
    ...
    public MethodResult onGrowthTick() {
        ...
        if (this.isCrossCrop() && AgriCraftConfig.crossOverChance > this.getRandom().nextDouble()) {
            ...
            this.spread();
            return MethodResult.SUCCESS;
            // (this.seed.getPlant().getGrowthChanceBase() + this.seed.getStat().getGrowth() * this.seed.getPlant().getGrowthChanceBonus()) * AgriCraftConfig.growthMultiplier > this.getRandom().nextDouble()
        } else if (te != null) {
            while (te.getTicksSinceUpdate() > seed.getPlant().getGrowthTimeBase()) {
                te.reduceCounter((long) seed.getPlant().getGrowthTimeBase());

                float temp = ClimateTFC.getActualTemp(world, pos, -te.getTicksSinceUpdate());
                float rainfall = ChunkDataTFC.getRainfall(world, pos);

                if(seed.getPlant().getGrowthRequirement().isValidForGrowth(temp, rainfall)) {
